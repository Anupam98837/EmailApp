...
<head>
  ...
  <!-- SheetJS for multi-sheet export -->
...
<body>
  <div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <div class="d-flex align-items-center gap-2">
        <label for="campaignSelect" class="mb-0 fw-semibold text-secondary">Select Campaign:</label>
        <select id="campaignSelect" class="form-select form-select-sm">
    ...
    <div class="tab-content">
        ...
        <div class="row gx-3">
          <div class="col-md-4">
            <div class="chart-card">
              <h6>Sent vs Opens (%)</h6>
              <canvas id="chartTotalOpen"></canvas>
            </div>
          </div>
          <div class="col-md-4">
            <div class="chart-card">
              <h6>Sent vs Clicks (%)</h6>
              <canvas id="chartTotalClick"></canvas>
            </div>
          </div>
          <div class="col-md-4">
            <div class="chart-card">
              <h6>Bounces Hard vs Soft</h6>
              <canvas id="chartBounces"></canvas>
            </div>
          </div>
        </div>
        <div id="overviewStats" class="row mt-4 gx-3">
          <div class="col-12 text-center py-4 text-muted">
            Select a campaign to load overview…
          </div>
        </div>
      </div>
      ...
      <div class="tab-pane fade" id="previewPane">
        <div class="template-preview bg-white p-3 rounded shadow-sm">
          <div id="previewLoading" class="text-center py-5 text-muted">
            Select a campaign to load preview…
          </div>
          <iframe id="templateIframe" style="width:100%; height:600px; border:none; display:none;"></iframe>
        </div>
      </div>
      ...
      <div class="tab-pane fade" id="detailedPane">
        <div class="filter-bar">
          ...
          <div class="d-flex align-items-center gap-2">
            <label class="mb-0">Engagement:</label>
        ...
        <div class="table-responsive bg-white p-3 rounded shadow-sm">
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody id="detailedBody">
              <tr>
                <td colspan="10" class="py-4 text-muted text-center">Select a campaign to load details…</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  <script>
    let campaignId = null,
        allRows = [],
        displayedRows = [],
        chartTotalOpen, chartTotalClick, chartBounces;
    ...
    function showLoadingStates(){
      $('#overviewStats').html('<div class="col-12 text-center py-4 text-muted">Loading overview…</div>');
      chartTotalOpen.data.datasets[0].data=[0,100]; chartTotalOpen.update();
      chartTotalClick.data.datasets[0].data=[0,100]; chartTotalClick.update();
      chartBounces.data.datasets[0].data=[0,0]; chartBounces.update();
      $('#previewLoading').show().text('Loading preview…');
      $('#templateIframe').hide().attr('srcdoc','');
      $('#detailedBody').html('<tr><td colspan="10" class="py-4 text-muted text-center">Loading details…</td></tr>');
    }

    function loadCampaignList(){
      $.ajax({
        url: `/api/campaign?page=1&per_page=1000`,
        headers: { Authorization: `Bearer ${token}` }
      }).done(res => {
        const sel = $('#campaignSelect').empty();
        sel.append('<option value=\"\">Select a campaign…</option>');
        res.data.data.forEach(c => {
          sel.append(`<option value=\"${c.id}\">${c.title}</option>`);
        });
        sel.on('change', () => {
          campaignId = sel.val();
    ...
    function loadOverview(){
      $.ajax({
        ...
        $('#overviewStats').html(`
          <div class="col-md-3"><div class="card-stat"><h6>Total Sent</h6><div class="value">${d.total_sent}</div></div></div>
          <div class="col-md-3"><div class="card-stat"><h6>Unique Opens</h6><div class="value">${d.unique_opens} <small>(${d.open_rate}%)</small></div></div></div>
          <div class="col-md-3"><div class="card-stat"><h6>Unique Clicks</h6><div class="value">${d.unique_clicks} <small>(${d.click_rate}%)</small></div></div></div>
          <div class="col-md-3"><div class="card-stat"><h6>Total Bounces</h6><div class="value">${d.total_bounces}</div></div></div>
        `);
      });
    }
    ...
    function loadPreview(){
      $.ajax({
        ...
        $('#previewLoading').hide();
        $('#templateIframe').attr('srcdoc', res.data.template_body_html).show();
      });
    }
    ...
    function applyFilters() {
      ...
      let filtered = status === 'all' ? [...allRows] : allRows.filter(r => r.status === status);
    ...
    function exportExcel(){
      ...
      const sheets = [
        ...
        ['Opened',            allRows.filter(r=>r.open_count>0)],
        ['Clicked',           allRows.filter(r=>r.click_count>0)],
        ['Unsubscribed',      allRows.filter(r=>r.unsubscribed)],
        ['Bounced',           allRows.filter(r=>r.bounced)],
        ['Skipped',           allRows.filter(r=>r.skip_reason)],
        ['Failed',            allRows.filter(r=>r.status==='failed')],
      ];
  ...
  Swal.fire({
    title: 'Export CSV',
    html: `<label><input type="checkbox" id="excludeUnsub"> Exclude unsubscribed rows</label>`,
    ...
    const rows = exclude
      ? displayedRows.filter(r => !r.unsubscribed)
      : displayedRows;
    ...
    a.href     = url;
    a.download = `campaign_${campaignId}_subscribers.csv`;
    ...
    $(function(){
      initCharts();
      loadCampaignList();
      $('#statusFilter, #engagementFilter').on('change', applyFilters);
